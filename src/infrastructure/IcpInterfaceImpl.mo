...
module {
    type Event = T.Event;
    type SubscriptionInfo = IIcpService.SubscriptionInfo;
    ...
    public class ICRC72ClientImpl(hubPrincipal : Principal, caller : Principal) : IIcpService.IIcpService {
        public func subscribe(namespace : Text) : async Bool {
            let broadcaster : BroadcasterActor = actor (Principal.toText(hubPrincipal));
            let sub_args : SubscriptionInfo = {
                namespace = namespace;
                subscriber = caller;
                active = true;
                filters = [namespace];
